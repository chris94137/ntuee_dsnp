...
class MemBlock
{
   ...
   bool getMem(size_t t, T*& ret) {
      // TODO
		size_t realSpace = toSizeT(t);
		if(realSpace > getRemainSize()) return false;
		void* p = _ptr;
		ret = (T*) p;
		_ptr += realSpace;
      return true;
...
class MemRecycleList
{
   ...
   T* popFront() {
      ...
      if(temp)
		{
			T** first_eight_bytes_ptr = (T**) temp;
			_first = *first_eight_bytes_ptr;
		}
      return temp;
   }
   ...
   void  pushFront(T* p) {
      // TODO
		T* temp = _first;
		_first = p;
		T** first_eight_bytes_ptr = (T**) _first;
		*first_eight_bytes_ptr = temp;
   }
   ...
   size_t numElm() const {
      // TODO
		size_t count = 0;
		T* current_ptr = _first;
		while(current_ptr)
		{
			count ++;
			T* temp = current_ptr;
			T** first_eight_bytes_ptr = (T**) temp;
			current_ptr = *first_eight_bytes_ptr;
		}
      return count;
   }
   ...
   T*                  _first;     // the first recycled data, point to the next element
...
public:
   ...
   void reset(size_t b = 0) {
      ...
      #endif // MEM_DEBUG
      // TODO
		MemBlock<T>* current_block = _activeBlock;
		while(current_block)
		{
			current_block->reset();
...
private:
   size_t                     _blockSize;
   MemBlock<T>*               _activeBlock;//the block that still have memory
   ...
   MemRecycleList<T>* getMemRecycleList(size_t n) {
      ...
      // TODO
		...
		while(currentList)
		{
			if(currentList->getArrSize() == n) return currentList;
         if(!currentList->getNextList()) break;
         currentList = currentList->getNextList();
		}
		MemRecycleList<T>* p = new MemRecycleList<T>(n);
		currentList->setNextList(p);
		
      return currentList->getNextList();
   }
   ...
   T* getMem(size_t t) {
      ...
      // TODO
		MemRecycleList<T>* current_recycleList = getMemRecycleList(n);
      #ifdef MEM_DEBUG
      cout << "Recycled from _recycleList[" << n << "]..." << current_recycleList << endl;
      #endif // MEM_DEBUG
      ...
      // TODO
      cerr << "current_recycleList->_first : " << current_recycleList->_first << endl;
		if(current_recycleList->_first == 0)
		{
			if(!_activeBlock->getMem(t, ret))
			{
            cerr << "!_activeBlock->getMem(t, ret)" << endl;
				T* p;
            size_t remainMem = _activeBlock->getRemainSize();
            cerr << "remainMem : " << remainMem << endl;
				if(remainMem >= S)
            {
               size_t rn = getArraySize(remainMem);
               _activeBlock->getMem(remainMem, p);
				   _recycleList[rn].pushFront(p);
               #ifdef MEM_DEBUG
               cout << "Recycling " << p << " to _recycleList[" << rn << "]\n";
               #endif // MEM_DEBUG
            }
				MemBlock<T>* b = new MemBlock<T>(_activeBlock, _blockSize);
				_activeBlock = b;
		...
		{
			ret = current_recycleList->popFront();
		}
